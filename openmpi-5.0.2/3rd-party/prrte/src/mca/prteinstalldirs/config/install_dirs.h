...
#ifndef PRTE_INST_DIRS_H
#define PRTE_INST_DIRS_H

#define PRTE_PREFIX "/opt/ThirdParty-11/platforms/linux64Gcc/openmpi-5.0.2"
#define PRTE_EXEC_PREFIX "${prefix}"

/* The directory for installing executable programs that users can
   run. */
#define PRTE_BINDIR "${exec_prefix}/bin"

/* The directory for installing executable programs that can be run
   from the shell, but are only generally useful to system
   administrators. */
#define PRTE_SBINDIR "${exec_prefix}/sbin"
...
#define PRTE_LIBEXECDIR "${exec_prefix}/libexec"

/* The root of the directory tree for read-only
   architecture-independent data files.

   See not about PRTE_DATADIR.  And you probably want that one, not
   this one.  This is one of those "building block" paths, that is
   really only used for defining other paths. */
#define PRTE_DATAROOTDIR "${prefix}/share"
...
#define PRTE_DATADIR "${datarootdir}"

/* $(datadir)/package-name/.  You probably want to use this instead of
   PRTE_DATADIR */
#define PRTE_PKGDATADIR "${datadir}/prte"
...
#define PRTE_SYSCONFDIR "${prefix}/etc"

/* The directory for installing architecture-independent data files
   which the programs modify while they run. */
#define PRTE_SHAREDSTATEDIR "${prefix}/com"
